...
def forward_propagation_with_dropout(X, parameters, keep_prob = 0.5):
    ...
    D1 = np.random.rand(*A1.shape)                   # Step 1
    D1 = (D1 < keep_prob).astype(np.float64)         # Step 2
    A1 = A1 * D1                                     # Step 3 (mask)
    A1 = A1 / keep_prob                              # Step 4 (inverted dropout scaling)
    ...
    D2 = np.random.rand(*A2.shape)                   # Step 1
    D2 = (D2 < keep_prob).astype(np.float64)         # Step 2
    A2 = A2 * D2                                     # Step 3
...
def backward_propagation_with_dropout(X, Y, cache, keep_prob):
    ...
    dA2 = dA2 * D2           # Step 1: apply mask
    dA2 = dA2 / keep_prob    # Step 2: scale survivors
    ...
    dA1 = dA1 * D1           # Step 1: apply mask
    dA1 = dA1 / keep_prob    # Step 2: scale survivors
