...
def iou(box1, box2):
    ...
    ## (≈ 3 lines)
    box1_area = max(0.0, (box1_x2 - box1_x1)) * max(0.0, (box1_y2 - box1_y1))
    box2_area = max(0.0, (box2_x2 - box2_x1)) * max(0.0, (box2_y2 - box2_y1))
    ...
    #### START CODE HERE
        ...
        if tf.shape(scores_label)[0] > 0:  # Check if there are any boxes to process
            ...
            nms_indices_label = tf.image.non_max_suppression(
                boxes_label,
                scores_label,
                max_output_size=tf.shape(scores_label)[0],
...
def yolo_eval(yolo_outputs, image_shape = (720, 1280), max_boxes=10, score_threshold=.6, iou_threshold=.5):
    ...
    boxes = scale_boxes(boxes, tf.cast(image_shape, tf.float32))
    ...
    scores, boxes, classes = yolo_non_max_suppression(scores, # Use scores
                                  boxes, # Use boxes
                                  classes, # Use classes
                                  max_boxes=max_boxes, # Use max boxes
